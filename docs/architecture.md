...
- **Optional full ADK run**: Use `run_beacon_pipeline` (or a manager-backed entrypoint) for scheduled/batch runs so Perception → Reflection runs with real tool calls and shared state.
- **Configurable optimization policy**: Make the Optimization Engine’s weights and constraints configurable via `memory_preferences` so operations teams can tune how aggressively Beacon trades off cost vs service vs risk reduction.
