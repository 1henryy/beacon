...
export const COMPANY_PROFILE = {
    company_name: "Beacon Manufacturing",
...
export async function runReasoningAgent(signalEvent: any) {
    const response = await ai.models.generateContent({
        model: "gemini-2.5-flash",
        contents: `Analyze this SignalEvent: ${JSON.stringify(signalEvent)}. Determine exposure for Beacon Manufacturing. Risk formula: risk_score = probability × exposure × impact. Output a RiskCase JSON.`,
        config: {
            responseMimeType: "application/json",
            responseSchema: {
                type: Type.OBJECT,
