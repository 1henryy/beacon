...
export default function CaseDetail() {
    ...
    const handleApprove = async (proposalId: string) => {
        try {
            await supabase.from('change_proposals').update({ status: 'approved', approved_by: 'Beacon Admin' }).eq('proposal_id', proposalId);
            fetchCaseDetails(id!);
        } catch (err) {
            console.error(err);
        }
                ...
                <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        {riskCase.alternative_plans ? (
                            ...
                            <p className="text-sm text-slate-500 italic">Beacon Agent is currently generating mitigation plans...</p>
                        )}
                    </div>
                </div>
            </div>
