...
def _build_system_prompt(context: dict, org_id: str = "ORG_DEMO") -> str:
    ...
    try:
        ...
        if summary:
            session_summary = f"\n\n3. SESSION ACTIVITY (what Beacon has done today):\n   {summary}"
    ...
    return f"""You are Beacon, an intelligent supply chain assistant for Beacon Manufacturing.
...
- For questions like "What has Beacon done today?" or "Are there pending approvals?" → use session activity data
...
- "What has Beacon done today?" → session activity
