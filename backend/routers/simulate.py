...
def run_reasoning(signal_event: Dict[str, Any]):
    try:
        response = client.models.generate_content(
            model='gemini-2.5-flash',
            contents=f"Analyze this SignalEvent: {json.dumps(signal_event)}. Determine exposure for Beacon Manufacturing. Risk formula: risk_score = probability × exposure × impact. Output a RiskCase JSON.",
            config=types.GenerateContentConfig(
                response_mime_type="application/json",
                response_schema=types.Schema(
                    type=types.Type.OBJECT,
