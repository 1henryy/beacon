...
### `beacon_manager.py`
- Main manager agent (`build_beacon_manager()`) - ADK LlmAgent that wraps all layer coordinators
...
In production, `backend/main.py` starts a background perception scheduler on app startup that calls `run_perception_with_manager` every `PERCEPTION_INTERVAL_SECONDS` seconds (default 900s) for `BEACON_COMPANY_ID` (default `ORG_DEMO`), and skips runs when fresh `signal_events` already exist.
