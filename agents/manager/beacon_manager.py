"""
BeaconManagerAgent - Central orchestrator above all agents and layers.
...
from agents.root_agent import build_beacon_root_agent

MANAGER_SYSTEM_PROMPT = """You are BeaconManager, the central orchestrator for Beacon's autonomous supply chain resilience system.
...
6. SESSION NARRATIVE
   ...
   The chatbot can read this to answer "what has Beacon done today?"
...
def build_beacon_manager() -> LlmAgent:
    """
    Builds the BeaconManagerAgent that orchestrates all layer coordinators.
    ...
    root_agent = build_beacon_root_agent()
    
    manager = LlmAgent(
        name="BeaconManagerAgent",
